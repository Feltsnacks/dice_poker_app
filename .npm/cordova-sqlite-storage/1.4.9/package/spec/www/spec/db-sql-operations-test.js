...
var mytests = function () {
  describe('Plugin: plugin-specific sql test(s)', function () {
    var pluginScenarioList = [isAndroid ? 'Plugin-implementation-default' : 'Plugin', 'Plugin-implementation-2'];
    ...
    for (var i = 0; i < pluginScenarioCount; ++i) {
      describe(pluginScenarioList[i] + ': db.executeSql test(s)', function () {
        ...
        it(
          suiteName + 'db.executeSql string result test with null for parameter argument array',
          function (done) {
            var db = openDatabase(
              'DB-sql-String-result-test-with-null-parameter-arg-array.db',
              '1.0',
              'Demo',
              DEFAULT_SIZE
            );
            ...
            db.executeSql(
              "SELECT UPPER('first') AS uppertext",
              null,
              function (rs) {
        ...
        it(
          suiteName + 'Inline db.executeSql US-ASCII String manipulation test with empty ([]) parameter list',
          function (done) {
            var db = openDatabase(
              'Inline-db-sql-US-ASCII-string-test-with-empty-parameter-list.db',
              '1.0',
              'Demo',
              DEFAULT_SIZE
            );
        ...
        it(
          suiteName + 'db.executeSql string result test with undefined for parameter argument array',
          function (done) {
            var db = openDatabase(
              'DB-sql-String-result-test-with-undefined-for-parameter-arg-array.db',
              '1.0',
              'Demo',
              DEFAULT_SIZE
            );
        ...
        it(
          suiteName + 'db.executeSql check SELECT TYPEOF(?) with [101] for parameter argument array',
          function (done) {
            var db = openDatabase('DB-sql-SELECT-TYPEOF-101.db', '1.0', 'Demo', DEFAULT_SIZE);
            expect(db).toBeDefined();

            db.executeSql(
              'SELECT TYPEOF(?) AS myresult',
              [101],
              function (rs) {
                expect(rs).toBeDefined();
                expect(rs.rows).toBeDefined();
                expect(rs.rows.length).toBe(1);
                if (isMac || isWKWebView) expect(rs.rows.item(0).myresult).toBe('real');
                else if (isAndroid && isImpl2) expect(rs.rows.item(0).myresult).toBe('text');
                else expect(rs.rows.item(0).myresult).toBe('integer');
                db.close(done, done);
              },
              function (error) {
                // NOT EXPECTED:
        ...
        it(
          suiteName + 'db.executeSql check SELECT ? with [101] for parameter argument array',
          function (done) {
            ...
            db.executeSql(
              ...
              function (rs) {
                ...
                expect(rs.rows).toBeDefined();
                expect(rs.rows.length).toBe(1);
                if (isAndroid && isImpl2) expect(rs.rows.item(0).myresult).toBe('101');
                else expect(rs.rows.item(0).myresult).toBe(101);
                db.close(done, done);
              },
              function (error) {
                // NOT EXPECTED:
        ...
        it(
          suiteName + 'db.executeSql check SELECT TYPEOF(?) with [123.456] for parameter argument array',
          function (done) {
            ...
            db.executeSql(
              ...
              function (rs) {
                ...
                expect(rs.rows).toBeDefined();
                expect(rs.rows.length).toBe(1);
                if (isAndroid && isImpl2) expect(rs.rows.item(0).myresult).toBe('text');
                else expect(rs.rows.item(0).myresult).toBe('real');
                db.close(done, done);
              },
              function (error) {
                // NOT EXPECTED:
        ...
        it(
          suiteName + 'db.executeSql check SELECT ? with [123.456] for parameter argument array',
          function (done) {
            ...
            db.executeSql(
              ...
              function (rs) {
                ...
                expect(rs.rows).toBeDefined();
                expect(rs.rows.length).toBe(1);
                if (isAndroid && isImpl2) expect(rs.rows.item(0).myresult).toBe('123.456');
                else expect(rs.rows.item(0).myresult).toBe(123.456);
                db.close(done, done);
              },
              function (error) {
                // NOT EXPECTED:
        ...
        it(
          suiteName + 'db.executeSql check SELECT TYPEOF(?) with [0] for parameter argument array',
          function (done) {
        ...
        it(
          suiteName + 'db.executeSql check SELECT ? with [0] for parameter argument array',
          function (done) {
            var db = openDatabase('DB-sql-SELECT-TYPEOF-0.db', '1.0', 'Demo', DEFAULT_SIZE);
            expect(db).toBeDefined();
            ...
            db.executeSql(
              ...
              function (rs) {
                ...
                expect(rs.rows).toBeDefined();
                expect(rs.rows.length).toBe(1);
                if (isAndroid && isImpl2) expect(rs.rows.item(0).myresult).toBe('0');
                else expect(rs.rows.item(0).myresult).toBe(0);
                db.close(done, done);
              },
              function (error) {
                // NOT EXPECTED:
        ...
        it(
          suiteName + 'db.executeSql check SELECT TYPEOF(?) with [null] for parameter argument array',
          function (done) {
            ...
            db.executeSql(
              ...
              function (rs) {
                ...
                expect(rs.rows.length).toBe(1);
                if (isAndroid && isImpl2) expect(rs.rows.item(0).myresult).toBe('text');
                else expect(rs.rows.item(0).myresult).toBe('null');
                db.close(done, done);
              },
              function (error) {
                // NOT EXPECTED:
        ...
        it(
          suiteName +
            'db.executeSql check SELECT TYPEOF(?) with [undefined] for parameter argument array [BROKEN on Windows]',
          function (done) {
        ...
        it(
          suiteName +
            'db.executeSql check SELECT TYPEOF(?) with [Infinity] for parameter argument array [TBD Android/iOS/macOS plugin result]',
          function (done) {
            if (isWP8) pending('SKIP for WP8'); // SKIP for now
            ...
            db.executeSql(
              ...
              function (rs) {
                ...
                expect(rs.rows.length).toBe(1);
                if (isAndroid && isImpl2) expect(rs.rows.item(0).myresult).toBe('text');
                else if (!isWindows && !isMac) expect(rs.rows.item(0).myresult).toBe('null');
                else expect(rs.rows.item(0).myresult).toBe('real');
                db.close(done, done);
              },
              function (error) {
        ...
        it(
          suiteName +
            'db.executeSql check SELECT ? with [Infinity] for parameter argument array [TBD Android/iOS/macOS plugin result]',
          function (done) {
            if (isWP8) pending('SKIP for WP8'); // SKIP for now
            ...
            db.executeSql(
              ...
              function (rs) {
                ...
                expect(rs.rows).toBeDefined();
                expect(rs.rows.length).toBe(1);
                if (isWindows) expect(rs.rows.item(0).myresult).toBe(Infinity);
                else if (isAndroid && isImpl2) expect(rs.rows.item(0).myresult).toBe('');
                else expect(rs.rows.item(0).myresult).toBe(null);
                db.close(done, done);
              },
              function (error) {
        ...
        it(
          suiteName +
            'db.executeSql check SELECT TYPEOF(?) with [NaN] for parameter argument array [TBD Android/iOS/macOS plugin result]',
          function (done) {
        ...
        it(
          suiteName + 'db.executeSql store numeric values and check',
          function (done) {
            ...
            db.executeSql(
              'SELECT data AS d1, TYPEOF(data) AS t1, ABS(data) AS a1, UPPER(data) as u1 FROM MyTable',
              [],
              function (rs) {
                expect(rs.rows).toBeDefined();
                expect(rs.rows.length).toBe(9);
                expect(rs.rows.item(0).d1).toBe(101);
                if (isMac || isWKWebView) expect(rs.rows.item(0).t1).toBe('real');
                else expect(rs.rows.item(0).t1).toBe('integer');
                expect(rs.rows.item(0).a1).toBe(101);
                if (isMac || isWKWebView) expect(rs.rows.item(0).u1).toBe('101.0');
                else expect(rs.rows.item(0).u1).toBe('101');
                expect(rs.rows.item(1).d1).toBe(-101);
                if (isMac || isWKWebView) expect(rs.rows.item(1).t1).toBe('real');
                else expect(rs.rows.item(1).t1).toBe('integer');
                expect(rs.rows.item(1).a1).toBe(101);
                if (isMac || isWKWebView) expect(rs.rows.item(1).u1).toBe('-101.0');
                else expect(rs.rows.item(1).u1).toBe('-101');
                ...
                if (isMac || isWKWebView) expect(rs.rows.item(4).t1).toBe('real');
                else expect(rs.rows.item(4).t1).toBe('integer');
                expect(rs.rows.item(4).a1).toBe(1234567890123);
                if (isMac || isWKWebView) expect(rs.rows.item(4).u1).toBe('1234567890123.0');
                else expect(rs.rows.item(4).u1).toBe('1234567890123');
                expect(rs.rows.item(5).d1).toBe(-1234567890123);
                if (isMac || isWKWebView) expect(rs.rows.item(5).t1).toBe('real');
                else expect(rs.rows.item(5).t1).toBe('integer');
                expect(rs.rows.item(5).a1).toBe(1234567890123);
                if (isMac || isWKWebView) expect(rs.rows.item(5).u1).toBe('-1234567890123.0');
                else expect(rs.rows.item(5).u1).toBe('-1234567890123');
                ...
                if (isMac || isWKWebView) expect(rs.rows.item(8).t1).toBe('real');
                else expect(rs.rows.item(8).t1).toBe('integer');
                expect(rs.rows.item(8).a1).toBe(0);
                if (isMac || isWKWebView) expect(rs.rows.item(8).u1).toBe('0.0');
                else expect(rs.rows.item(8).u1).toBe('0');
                done();
              }
            );
          },
        ...
        it(
          suiteName + 'db.executeSql store null/undefined values and check [store undefined value BROKEN on Windows]',
          function (done) {
            if (isWP8) pending('SKIP for WP8'); // SKIP for now
            ...
            db.executeSql(
              ...
              function (rs) {
                ...
                if (isWindows)
                  // [FUTURE TBD]
                  expect(rs.rows.length).toBe(1);
                else expect(rs.rows.length).toBe(2);
        ...
        it(
          suiteName + 'db.executeSql store Infinity/NaN values and check [TBD Android/iOS plugin result]',
          function (done) {
            if (isWP8) pending('SKIP for WP8'); // SKIP for now
            if (isMac) pending('SKIP for macOS [CRASH]'); // FUTURE TBD
        ...
        it(
          suiteName + 'db.executeSql string result test with new String for SQL',
          function (done) {
            var db = openDatabase('DB-sql-new-String-test.db', '1.0', 'Demo', DEFAULT_SIZE);
            expect(db).toBeDefined();

            db.executeSql(
              new String("SELECT UPPER('first') AS uppertext"),
              null,
              function (rs) {
                expect(rs).toBeDefined();
                expect(rs.rows).toBeDefined();
        ...
        it(
          suiteName +
            'SELECT UPPER(?) AS upper1, UPPER(?) AS upper2 with "naive" Array subclass (constructor NOT explicitly set to subclasss) as value arguments array',
          function (done) {
            ...
            db.executeSql(
              'SELECT UPPER(?) AS upper1, UPPER(?) AS upper2',
              myObject,
              function (rs) {
                expect(rs).toBeDefined();
                expect(rs.rows).toBeDefined();
        ...
        it(
          suiteName +
            'SELECT UPPER(?) AS upper1, UPPER(?) AS upper2 with "naive" Array subclass (constructor explicitly set to subclasss) as value arguments array',
          function (done) {
        ...
        it(
          suiteName + 'db.executeSql string result test with dynamic object for SQL [INCONSISTENT BEHAVIOR]',
          function (done) {
            // MyDynamicObject "class":
            function MyDynamicObject() {
              this.name = 'Alice';
            }
            MyDynamicObject.prototype.toString = function () {
              return "SELECT UPPER('" + this.name + "') as uppertext";
            };
            ...
            var db = openDatabase(
              'DB-sql-string-result-test-with-dynamic-object-for-sql.db',
              '1.0',
              'Demo',
              DEFAULT_SIZE
            );
        ...
        it(
          suiteName + 'db.executeSql string result test with dynamic object for parameter arg [INCONSISTENT BEHAVIOR]',
          function (done) {
            // MyDynamicParameterObject "class":
            function MyDynamicParameterObject() {
              this.name = 'Alice';
            }
            MyDynamicParameterObject.prototype.toString = function () {
              return this.name;
            };
            ...
            // called exactly once, with the proper result:
            var db = openDatabase(
              'DB-sql-string-result-test-with-dynamic-object-for-parameter-arg.db',
              '1.0',
              'Demo',
              DEFAULT_SIZE
            );
        ...
        it(
          suiteName + 'Multi-row INSERT with parameters in db.executeSql test',
          function (done) {
            ...
            db.executeSql(
              ...
              function (rs) {
                ...
                if (!(isAndroid && isImpl2)) expect(rs.rowsAffected).toBe(2);
        ...
        test_it(
          suiteName + 'Multiple db.executeSql string result test',
          function () {
            // NOTE: this test checks that for db.executeSql(), the result callback is
            // called exactly once, with the proper result:
  ...
  describe('Plugin: plugin-specific error test(s)', function () {
    ...
    for (var i = 0; i < pluginScenarioCount; ++i) {
      describe(pluginScenarioList[i] + ': db.executeSql error test(s)', function () {
        ...
        it(
          suiteName + 'db.executeSql() with no arguments and then inline string test',
          function (done) {
        ...
        it(
          suiteName + 'db.executeSql error test with null for SQL statement',
          function (done) {
            ...
            db.executeSql(
              ...
              function (error) {
                ...
                // SKIP for now
                else if (isWindows) expect(error.code).toBe(0);
                else expect(error.code).toBe(5);

                if (isWP8) expect(true).toBe(true);
                // SKIP for now
                else if (isWindows) expect(error.message).toMatch(/Error preparing an SQLite statement/);
                else expect(error.message).toMatch(/near \"SLCT\": syntax error/);
              }
            );
        ...
        it(
          suiteName + 'db.executeSql error test with undefined for SQL statement',
          function (done) {
        ...
        it(
          suiteName + 'db.executeSql error test with true for SQL statement',
          function (done) {
            ...
            db.executeSql(
              ...
              function (error) {
                ...
                else expect(error.message).toMatch(/near \"true\": syntax error/);
        ...
        it(
          suiteName + 'db.executeSql error test with false for SQL statement',
          function (done) {
            ...
            db.executeSql(
              ...
              function (error) {
                ...
                else expect(error.message).toMatch(/near \"false\": syntax error/);
        ...
        it(
          suiteName + 'db.executeSql error test with Infinity for SQL statement',
          function (done) {
            ...
            db.executeSql(
              ...
              function (error) {
                ...
                else expect(error.message).toMatch(/near \"Infinity\": syntax error/);
          ...
          function (done) {
            ...
            db.executeSql(
              ...
              function (error) {
                ...
                else expect(error.message).toMatch(/near \"-\": syntax error/);
        ...
        it(
          suiteName + 'db.executeSql error test with NaN for SQL statement',
          function (done) {
            ...
            db.executeSql(
              ...
              function (error) {
                ...
                else expect(error.message).toMatch(/near \"NaN\": syntax error/);
        ...
        it(
          suiteName + 'db.executeSql error test with null for parameter argument array',
          function (done) {
            var db = openDatabase(
              'DB-execute-sql-error-test-with-null-for-parameter-arg-array.db',
              '1.0',
              'Demo',
              DEFAULT_SIZE
            );
        ...
        it(
          suiteName + 'db.executeSql error test with undefined for parameter argument array',
          function (done) {
            var db = openDatabase(
              'DB-execute-sql-error-test-with-undefined-for-parameter-arg-array.db',
              '1.0',
              'Demo',
              DEFAULT_SIZE
            );
        ...
        it(
          suiteName + 'db.executeSql error test with "string-value" for parameter argument array',
          function (done) {
            var db = openDatabase(
              'DB-execute-sql-error-test-with-string-value-for-parameter-arg-array.db',
              '1.0',
              'Demo',
              DEFAULT_SIZE
            );
        ...
        it(
          suiteName + 'db.executeSql error test with false for parameter argument array',
          function (done) {
            var db = openDatabase(
              'DB-execute-sql-error-test-with-false-for-parameter-arg-array.db',
              '1.0',
              'Demo',
              DEFAULT_SIZE
            );
        ...
        it(
          suiteName + 'db.executeSql() error test with "string-value" for success callback',
          function (done) {
        ...
        it(
          suiteName + 'db.executeSql() error test with "string-value" for error callback and then inline string test',
          function (done) {
        ...
        it(suiteName + 'Multiple db.executeSql error result test', function (done) {
          ...
          // First: syntax error
          db.executeSql(
            "SELCT upper('first') AS uppertext",
            [],
            function () {
              // NOT EXPECTED:
              expect(false).toBe(true);
            ...
            function (error) {
              ...
              else expect(error.message).toMatch(/near \"SELCT\": syntax error/);
          ...
          db.executeSql(
            "SELECT uper('second') as uppertext",
            [],
            function () {
              // NOT EXPECTED:
              expect(false).toBe(true);
            ...
            function (error) {
              ...
              else expect(error.message).toMatch(/no such function: uper/);
      ...
      describe(pluginScenarioList[i] + ': more db.executeSql test(s)', function () {
        ...
        test_it(suiteName + 'PRAGMAs & multiple database transactions mixed together', function () {
          ...
          db2.transaction(function (tx) {
            ...
            tx.executeSql(
              'CREATE TABLE IF NOT EXISTS tt2 (id2 integer primary key, data2 text, data_num2 integer)',
              [],
              function () {
