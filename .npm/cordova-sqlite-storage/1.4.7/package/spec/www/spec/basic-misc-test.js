...
var mytests = function () {
  ...
  for (var i = 0; i < scenarioCount; ++i) {
    describe(scenarioList[i] + ': basic/misc. db tx/sql feature test(s)', function () {
      ...
      it(
        suiteName + 'db readTransaction with a WITH clause',
        function (done) {
      ...
      it(
        suiteName + 'create virtual table using FTS3',
        function (done) {
      ...
      it(
        suiteName + 'create virtual table using FTS4',
        function (done) {
          if (isWP8) pending('NOT IMPLEMENTED for WP(8)'); // NOT IMPLEMENTED in CSharp-SQLite
      ...
      // Test for Cordova-sqlcipher-adapter version (SQLCipher 3.4.0 based on SQLite 3.11.0)
      it(
        suiteName + 'JSON1 json_object test',
        function (done) {
          //if (isWebSql) pending('SKIP for Web SQL (not implemented)');
          pending('SKIP: NOT IMPLEMENTED for this version');
          ...
          db.transaction(function (tx) {
            ...
            // Derived from sample in: https://www.sqlite.org/json1.html
            tx.executeSql(
              'SELECT json_object(?,?) AS my_object;',
              ['ex', '[52,3.14159]'],
              function (tx, res) {
                // EXPECTED RESULT:
      ...
      // Test for Cordova-sqlcipher-adapter version (SQLCipher 3.4.0 based on SQLite 3.11.0)
      it(
        suiteName + 'create virtual table using FTS5',
        function (done) {
          //if (isWebSql) pending('SKIP for Web SQL (not implemented)');
          pending('SKIP: NOT IMPLEMENTED for this version');
      ...
      it(
        suiteName + 'create virtual table using R-Tree',
        function (done) {
          if (isWebSql) pending('SKIP for Web SQL');
          if (isWP8) pending('NOT IMPLEMENTED for WP(8)'); // NOT IMPLEMENTED in CSharp-SQLite
  ...
  for (var i = 0; i < scenarioCount; ++i) {
    describe(scenarioList[i] + ': basic/misc tx error handling test(s)', function () {
      ...
      describe(scenarioList[i] + ': basic tx error semantics test(s)', function () {
        ...
        it(
          suiteName + 'syntax error handler returns undefined (deviation in WebKit Web SQL implementation)',
          function (done) {
            ...
            db.transaction(
              ...
              function () {
                ...
                if (isWebSql) expect(true).toBe(true);
      ...
      describe(scenarioList[i] + ': other tx error handling test(s)', function () {
        ...
        it(
          suiteName + 'empty transaction (no callback argument) and then SELECT transaction',
          function (done) {
        ...
        it(
          suiteName + 'empty readTransaction (no callback argument) and then SELECT transaction',
          function (done) {
        ...
        it(
          suiteName + 'transaction.executeSql on BOGUS number',
          function (done) {
        ...
        it(
          suiteName + 'transaction.executeSql with no executeSql argument (BOGUS)',
          function (done) {
