(function () {
  ...
  Statement = WinJS.Class.define(
    function (db, sql, args) {
      try {
  ...
  Database = WinJS.Class.define(
    ...
    },
    {
      ...
      one: function (sql, args) {
        var row,
          statement = this.prepare(sql, args);
  ...
  ItemDataSource = WinJS.Class.derive(
    ...
    function (db, sql, args, keyColumnName, groupKeyColumnName) {
      var dataAdapter = {
        ...
        itemsFromIndex: function (requestIndex, countBefore, countAfter) {
          ...
          items = db.map('SELECT * FROM (' + sql + ') LIMIT ' + limit + ' OFFSET ' + offset, function (row) {
            var item = {
  ...
  GroupDataSource = WinJS.Class.derive(
    WinJS.UI.VirtualizedDataSource,
