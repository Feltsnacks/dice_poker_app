...
var mytests = function () {
  for (var i = 0; i < scenarioCount; ++i) {
    describe(scenarioList[i] + ': tx semantics test(s)', function () {
      ...
      it(
        suiteName + 'Simple tx sql order test',
        function (done) {
      ...
      it(
        suiteName + 'Simple tx sql order test with error recovery',
        function (done) {
      ...
      test_it(suiteName + 'transaction test: check rowsAffected [intermediate]', function () {
        ...
        function test3(tx) {
          tx.executeSql('INSERT INTO characters VALUES (?,?,?)', ['Mega Man', 'Capcom', 0], function (tx, res) {
            equal(res.rowsAffected, 1);
            tx.executeSql(
              'UPDATE characters SET fav=?, name=? WHERE creator=?;',
              [1, 'X', 'Capcom'],
              function (tx, res) {
                equal(res.rowsAffected, 1);
                tx.executeSql(
                  'UPDATE characters SET fav=? WHERE (creator=? OR creator=?)',
                  [1, 'Capcom', 'Nintendo'],
                  function (tx, res) {
                    equal(res.rowsAffected, 3);
          ...
          tx.executeSql('INSERT or IGNORE INTO characters VALUES (?,?,?)', ['Sonic', 'Sega', 0], function (tx, res) {
            ...
            tx.executeSql('INSERT INTO characters VALUES (?,?,?)', ['Tails', 'Sega', 0], function (tx, res) {
              ...
              tx.executeSql('INSERT INTO companies VALUES (?,?)', ['Sega', 1], function (tx, res) {
                ...
                // query with subquery
                var sql =
                  'UPDATE characters ' + ' SET fav=(SELECT fav FROM companies WHERE name=?)' + ' WHERE creator=?';
          ...
          try {
            db.transaction(
              ...
              function (error) {
                ...
                else expect(error.message).toBe('boom');
