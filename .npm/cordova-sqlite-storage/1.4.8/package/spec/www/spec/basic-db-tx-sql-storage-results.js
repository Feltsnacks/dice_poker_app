...
var mytests = function () {
  for (var i = 0; i < scenarioCount; ++i) {
    describe(scenarioList[i] + ': BASIC db tx sql results test(s)', function () {
      ...
      it(
        suiteName + 'Simple INSERT results test: check insertId & rowsAffected in result',
        function (done) {
      ...
      it(
        suiteName + 'basic db sql transaction results test',
        function (done) {
                ...
                function (tx, res) {
                  ...
                  db.transaction(
                    function (tx) {
                      ...
                      tx.executeSql(
                        'UPDATE test_table SET data_num = ? WHERE data_num = 100',
                        [101],
                        function (tx, res) {
                          ++check_count;
      ...
      it(
        suiteName + 'db transaction result object lifetime',
        function (done) {
                    ...
                    function (tx) {
                      ...
                      tx.executeSql('SELECT * FROM test_table;', [], function (tx, res) {
                        ...
                        if (isWebSql) {
                          ...
                          if (!isAndroid || /Android [1-4]/.test(navigator.userAgent)) expect(temp1.data).toBe('test');
      ...
      it(
        suiteName + 'tx sql starting with extra space results test',
        function (done) {
      ...
      it(
        suiteName + 'tx sql starting with extra semicolon results test',
        function (done) {
          // [BUG #458] BROKEN for androidDatabaseImplementation: 2 (built-in android.database) setting
          if (isWP8) pending('BROKEN for WP8');
          if (isAndroid && isImpl2)
            pending('BROKEN for androidDatabaseImplementation: 2 (built-in android.database) setting');
      ...
      describe(
        scenarioList[i] + ': NON-standard Multi-row INSERT with parameters (post-sqlite 3.6 feature)]',
        function () {
          it(
            suiteName +
              'Multi-row INSERT with parameters - DEVIATION: (post-sqlite 3.6 feature)' +
                ...
                function (tx) {
                  ...
                  tx.executeSql('INSERT INTO TestTable VALUES (?,?),(?,?)', ['a', 1, 'b', 2], function (ignored1, rs1) {
                    ...
                    if (isWebSql || !(isAndroid && isImpl2))
                      // [rowsAffected BROKEN for built-in (AOSP) android.database]
                      expect(rs1.rowsAffected).toBe(2);
                    else expect(rs1.rowsAffected).toBe(1); // [ACTUAL (BROKEN) for built-in (AOSP) android.database]
      ...
      describe(suiteName + 'NON-STANDARD SQL statement list test(s)', function () {
        it(
          suiteName +
            'INSERT statement list (NOT covered by Web SQL standard) - ' +
            (isWebSql ? 'Web SQL ERROR' : 'DEVIATION - PLUGIN BROKEN (with potential data loss)'),
          function (done) {
            ...
            db.transaction(
              ...
              function (error) {
                ...
                if (!isWebSql) expect('Plugin behavior changed').toBe('--');
                ...
                if (isWebSql) expect(error.message).toMatch(/could not prepare statement.*1 not an error/);
              ...
              function () {
                ...
                if (isWebSql) expect('Unexpected result for Web SQL').toBe('--');
        ...
        it(
          suiteName + 'executeSql with SELECT statement list - NOT ALLOWED [PLUGIN BROKEN]',
          function (done) {
            ...
            db.transaction(function (tx) {
              tx.executeSql(
                ...
                function (ignored, rs) {
                  ...
                  if (isWebSql) expect('WebKit Web SQL implementation changed (DEVIATION)').toBe('--');
                  else expect(rs).toBeDefined();

                  // EXTRA for INVESTIGATION: statement list with syntax error after the first statement
                  tx.executeSql(
                    'SELECT 1; SLCT 2',
                    [],
                    function (ignored1, rs2) {
                ...
                function (ignored, error) {
                  ...
                  return isWebSql ? done() || false : db.close(done, done) || false;
                }
              );
            });
          },
      ...
      describe(suiteName + 'Binary BLOB data INSERT test(s)', function () {
        it(
          suiteName + "INSERT Binary literal BLOB data (X'010203'), check results, and check stored data HEX value",
          function (done) {
      ...
      describe(suiteName + 'STANDARD multi-row INSERT tests', function () {
        it(
          suiteName +
            'INSERT multiple rows from with SELECT; check results & check stored data [rowsAffected INCORRECT with androidDatabaseImplementation: 2 (built-in android.database) setting]',
          function (done) {
            ...
            db.transaction(
              function (tx) {
                ...
                tx.executeSql('INSERT INTO tt2 SELECT data FROM tt1;', [], function (ignored1, rs1) {
                  ...
                  if (!(isAndroid && isImpl2)) expect(rs1.rowsAffected).toBe(2);
                  else expect(rs1.rowsAffected).toBe(1);
        ...
        it(
          suiteName +
            'INSERT with TRIGGER & check results [rowsAffected INCORRECT with androidDatabaseImplementation: 2 (built-in android.database) setting]',
          function (done) {
            ...
            db.transaction(
              function (tx) {
                ...
                tx.executeSql(
                  "CREATE TRIGGER t1 AFTER INSERT ON tt1 BEGIN INSERT INTO tt2 VALUES(datetime('now')); END;"
                );
      ...
      describe(suiteName + 'ALTER TABLE tests', function () {
        it(
          suiteName + 'ALTER TABLE ADD COLUMN test',
          function (done) {
        ...
        it(
          suiteName + 'ALTER TABLE RENAME test',
          function (done) {
