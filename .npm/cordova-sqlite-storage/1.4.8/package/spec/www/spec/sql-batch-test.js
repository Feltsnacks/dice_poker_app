  ...
  for (var i = 0; i < pluginScenarioCount; ++i) {
    describe(pluginScenarioList[i] + ': BATCH SQL test(s)', function () {
      ...
      describe(pluginScenarioList[i] + ': Basic sql batch test(s)', function () {
        it(
          suiteName + 'Single-column batch sql test',
          function (done) {
        ...
        it(
          suiteName + 'Single-column batch sql test values: INSERT INTEGER/REAL number values and check stored data',
          function (done) {
              ...
              function () {
                db.executeSql(
                  ...
                  function (rs) {
                    ...
                    if (isMac || isWKWebView) expect(rs.rows.item(6).t1).toBe('real');
                    else expect(rs.rows.item(6).t1).toBe('integer');
                    expect(rs.rows.item(6).a1).toBe(0);
                    if (isMac || isWKWebView) expect(rs.rows.item(6).u1).toBe('0.0');
                    else expect(rs.rows.item(6).u1).toBe('0');
                    done();
                  }
                );
              },
        ...
        it(
          suiteName +
            'Single-column batch sql test values: INSERT null/undefined values and check stored data [BROKEN on Windows]',
          function (done) {
        ...
        it(
          suiteName +
            'Single-column batch sql test values: INSERT +/- Infinity & NaN values and check stored data [TBD Android/iOS/macOS plugin result for +/- Infinity]',
          function (done) {
            if (isWP8) pending('SKIP for WP8'); // SKIP for now
        ...
        it(
          suiteName + 'batch sql with dynamic object for SQL [INCONSISTENT BEHAVIOR]',
          function (done) {
            ...
            MyDynamicObject.prototype.toString = function () {
              return "INSERT INTO MyTable VALUES ('" + this.name + "')";
            };
            ...
            myObject.name = 'Betty';
            db.sqlBatch(
              ['DROP TABLE IF EXISTS MyTable', 'CREATE TABLE MyTable (data)', myObject],
              function () {
        ...
        it(
          suiteName + 'batch sql with dynamic object for SQL arg value [INCONSISTENT BEHAVIOR]',
          function (done) {
        ...
        it(
          suiteName + 'Multi-row INSERT with parameters in batch sql test',
          function (done) {
        ...
        it(
          suiteName + 'batch sql with syntax error',
          function (done) {
        ...
        it(
          suiteName + 'batch sql with constraint violation (check error code & basic error message pattern)',
          function (done) {
            ...
            db.sqlBatch(
              ...
              function (error) {
                ...
                else expect(error.message).toMatch(/a statement with no error handler failed.*constraint fail/);
        ...
        it(
          suiteName + 'batch sql failure-safe semantics',
          function (done) {
