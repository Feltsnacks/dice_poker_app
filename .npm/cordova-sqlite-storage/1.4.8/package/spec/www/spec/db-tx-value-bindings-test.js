  ...
  for (var i = 0; i < scenarioCount; ++i) {
    describe(scenarioList[i] + ': tx INSERT value bindings test(s)', function () {
      ...
      describe(suiteName + 'transaction column value insertion test(s)', function () {
        it(
          suiteName + 'INSERT with null parameter argument value and check stored data',
          function (done) {
            ...
            db.transaction(
              function (tx) {
                ...
                tx.executeSql(
                  'CREATE TABLE IF NOT EXISTS test_table (data1, data2)',
                  [],
                  function (ignored1, ignored2) {
                    tx.executeSql(
                      'INSERT INTO test_table VALUES (?,?)',
                      [null, 'test-string'],
                      function (ignored, rs1) {
                        ...
                        tx.executeSql('SELECT * FROM test_table', [], function (ignored, rs2) {
                          ...
                          tx.executeSql(
                            'SELECT TYPEOF(data1) AS t1, TYPEOF(data2) AS t2 FROM test_table',
                            null,
                            function (ignored, rs3) {
        ...
        it(
          suiteName +
            'INSERT with undefined parameter argument value (inserted as null) and check stored data [BROKEN for Windows: Unsupported argument type ERROR]',
          function (done) {
                    ...
                    tx.executeSql(
                      ...
                      function (ignored, rs1) {
                        ...
                        tx.executeSql('SELECT * FROM test_table', [], function (ignored, rs2) {
                          ...
                          if (isWebSql && isAndroid) expect(row.data1).toBe('undefined');
                          else expect(row.data1).toBeNull();
                          expect(row.data2).toBe('test-string');
                          ...
                          tx.executeSql(
                            ...
                            function (ignored, rs3) {
                              ...
                              expect(rs3.rows).toBeDefined();
                              expect(rs3.rows.length).toBe(1);
                              if (isWebSql && isAndroid) expect(rs3.rows.item(0).t1).toBe('text');
                              else expect(rs3.rows.item(0).t1).toBe('null');
                              expect(rs3.rows.item(0).t2).toBe('text');
        ...
        it(
          suiteName + "all columns should be included in result set including id integer primary key & 'null' columns",
          function (done) {
        ...
        it(
          suiteName +
            'INSERT integer value (42) with no/INTEGER/NUMERIC/REAL/TEXT affinity & check stored data [TBD Plugin vs (WebKit) Web SQL]',
          function (done) {
            ...
            db.transaction(
              function (tx) {
                ...
                tx.executeSql(
                  'CREATE TABLE IF NOT EXISTS tt (data1, data2 INTEGER, data3 NUMERIC, data4 REAL, data5 TEXT)',
                  null,
                  function (ignored1, ignored2) {
                    tx.executeSql('INSERT INTO tt VALUES (?,?,?,?,?)', [42, 42, 42, 42, 42], function (ignored, rs1) {
                      expect(rs1).toBeDefined();
                      expect(rs1.rowsAffected).toBe(1);
                      ...
                      tx.executeSql('SELECT * FROM tt', [], function (ignored, rs2) {
                        ...
                        if (isWebSql || isMac || isWKWebView) expect(row.data5).toBe('42.0');
                        else expect(row.data5).toBe('42');

                        tx.executeSql(
                          'SELECT TYPEOF(data1) AS t1, TYPEOF(data2) AS t2, TYPEOF(data3) AS t3, TYPEOF(data4) AS t4, TYPEOF(data5) AS t5 FROM tt',
                          [],
                          function (ignored, rs3) {
                            expect(rs3).toBeDefined();
                            expect(rs3.rows).toBeDefined();
                            ...
                            if (isWebSql || isMac || isWKWebView) expect(row.t1).toBe('real');
                            else expect(row.t1).toBe('integer');
        ...
        it(
          suiteName +
            'INSERT Infinity with no/NUMERIC/REAL/INTEGER/TEXT type affinity and check stored data [Android/iOS Plugin BROKEN: stored with null value]',
          function (done) {
            if (isWP8) pending('SKIP for WP8'); // SKIP for now
            ...
            db.transaction(
              function (tx) {
                tx.executeSql('DROP TABLE IF EXISTS test_table');
                tx.executeSql(
                  'CREATE TABLE test_table (data, data_num NUMERIC, data_real REAL, data_int INTEGER, data_text TEXT)',
                  [],
                  function (ignored1, ignored2) {
                    tx.executeSql(
                      'INSERT INTO test_table VALUES (?,?,?,?,?)',
        ...
        it(
          suiteName + 'INSERT NaN with no/NUMERIC/REAL/INTEGER/TEXT type affinity and check stored data',
          function (done) {
            var db = openDatabase('INSERT-minus-Infinity-and-check.db', '1.0', 'Demo', DEFAULT_SIZE);
        ...
        it(
          suiteName +
            'INSERT TEXT string with emoji [\\u1F603 SMILING FACE (MOUTH OPEN)], SELECT the data, and check' +
            (!isWebSql && (isWindows || (isAndroid && !isImpl2))
              ? ' [BROKEN: SELECT HEX(data) result on Android-sqlite-connector/Windows]'
              : ''),
          function (done) {
        ...
        it(suiteName + 'number values inserted using number bindings', function (done) {
          ...
          db.transaction(
            function (tx) {
              ...
              tx.executeSql(
                'CREATE TABLE IF NOT EXISTS test_table (id integer primary key, data_text1, data_text2, data_int, data_real)'
              );
            },
            function (err) {
              expect(false).toBe(true);
            ...
            function () {
              db.transaction(function (tx) {
                tx.executeSql(
                  'insert into test_table (data_text1, data_text2, data_int, data_real) VALUES (?,?,?,?)',
                  ['314159', '3.14159', 314159, 3.14159],
                  function (tx, res) {
                    expect(res).toBeDefined();
                    expect(res.rowsAffected).toBe(1);
                    ...
                    tx.executeSql('select * from test_table', [], function (tx, res) {
                      ...
                      tx.executeSql(
                        'SELECT TYPEOF(data_text1) AS t1, TYPEOF(data_text2) AS t2, data_int AS t3, data_real AS t4',
                        null,
                        function (ignored, rs3) {
                          expect(rs3).toBeDefined();
                          expect(rs3.rows).toBeDefined();
        ...
        it(
          suiteName + 'BIG INTEGER INSERT value bindings',
          function (done) {
            ...
            db.transaction(
              function (tx) {
                ...
                tx.executeSql(
                  'CREATE TABLE IF NOT EXISTS tt (data1, test_int INTEGER, test_num NUMERIC, test_date DATETIME, test_text TEXT)',
                  null,
                  function (ignored1, ignored2) {
                    tx.executeSql(
                      'INSERT INTO tt VALUES (?,?,?,?,?)',
                      [1424174959894, 1424174959894, 1424174959894, 1424174959894, 1424174959894],
                      function (ignored, rs1) {
                        expect(rs1).toBeDefined();
                        expect(rs1.rowsAffected).toBe(1);
                        expect(rs1.insertId).toBe(1);
                        ...
                        tx.executeSql('SELECT * FROM tt', [], function (ignored, rs2) {
                          ...
                          if (isWebSql || isMac || isWKWebView) expect(row.test_text).toBe('1424174959894.0');
                          // ([Big] number inserted as string ok)
                          else expect(row.test_text).toBe('1424174959894'); // (Big integer number inserted as string ok)

                          tx.executeSql(
                            'SELECT TYPEOF(data1) AS t1, TYPEOF(test_int) AS t2, TYPEOF(test_num) AS t3, TYPEOF(test_date) AS t4, TYPEOF(test_text) AS t5 FROM tt',
                            [],
                            function (ignored, rs3) {
                              expect(rs3).toBeDefined();
                              expect(rs3.rows).toBeDefined();
        ...
        it(
          suiteName + 'BIG REAL INSERT value bindings',
          function (done) {
            ...
            db.transaction(
              function (tx) {
                tx.executeSql('DROP TABLE IF EXISTS tt');
                tx.executeSql(
                  'CREATE TABLE IF NOT EXISTS tt (data1 REAL, data2 NUMERIC, data3 INTEGER, data4 TEXT)',
                  null,
                  function (ignored1, ignored2) {
                    tx.executeSql(
                      'INSERT INTO tt VALUES (?,?,?,?)',
                      [1234567890123.4, 1234567890123.4, 1234567890123.4, 1234567890123.4],
                      function (tx, rs1) {
                        expect(rs1).toBeDefined();
                        expect(rs1.rowsAffected).toBe(1);
                        expect(rs1.insertId).toBe(1);
                        ...
                        tx.executeSql('SELECT * FROM tt', [], function (tx, rs2) {
                          ...
                          tx.executeSql(
                            'SELECT TYPEOF(data1) AS t1, TYPEOF(data2) AS t2, TYPEOF(data3) AS t3, TYPEOF(data4) AS t4 FROM tt',
                            [],
                            function (tx, rs3) {
                              expect(rs3).toBeDefined();
                              expect(rs3.rows).toBeDefined();
        ...
        it(suiteName + 'executeSql parameter as array', function (done) {
          ...
          db.transaction(
            ...
            function () {
              db.transaction(function (tx) {
                tx.executeSql(
                  'insert into test_table (data1, data2) VALUES (?,?)',
                  ['abc', [1, 2, 3]],
                  function (tx, res) {
                    expect(res).toBeDefined();
                    expect(res.rowsAffected).toBe(1);
        ...
        it(
          suiteName +
            "INSERT inline BLOB value (X'40414243') and check stored data [SELECT BLOB ISSUE with androidDatabaseImplementation: 2 & Windows/WP8]",
          function (done) {
                    ...
                    tx.executeSql(
                      ...
                      function (ignored, error) {
                        if ((!isWebSql && isWindows) || (isAndroid && isImpl2)) {
                          ...
                          if (isWindows) expect(error.message).toMatch(/Unsupported column type in column 0/);
                          else expect(error.message).toMatch(/unknown error.*code 0.*Unable to convert BLOB to string/);
                        } else {
                          // NOT EXPECTED:
                          expect(false).toBe(true);
                          expect(error.message).toBe('---');
      ...
      describe(suiteName + 'parameter count mismatch tests', function () {
        it(
          suiteName + 'executeSql with not enough parameters (Plugin DEVIATION: does not reject such SQL statements)',
          function (done) {
        ...
        it(
          suiteName + 'executeSql with too many parameters [extra TEXT string]',
          function (done) {
                ...
                db.transaction(function (tx) {
                  tx.executeSql(
                    ...
                    function (ignored, error) {
                      ...
                      if (isWebSql) expect(error.code).toBe(5);
                      ...
                      else if (isWindows) expect(error.message).toMatch(/Error 25 when binding argument to SQL query/);
                      else expect(error.message).toMatch(/index.*out of range/);
        ...
        it(
          suiteName + 'executeSql with too many parameters [extra REAL value]',
          function (done) {
            if (isWP8) pending('SKIP for WP8'); // TBD BROKEN on WP8
        ...
        it(
          suiteName + 'executeSql with too many parameters [extra INTEGER value]',
          function (done) {
            if (isWP8) pending('SKIP for WP8'); // TBD BROKEN on WP8
        ...
        it(
          suiteName + 'executeSql with too many parameters [extra NULL value]',
          function (done) {
            if (isWP8) pending('SKIP for WP8'); // TBD BROKEN on WP8
      ...
      describe(scenarioList[i] + ': extra tx column value binding test(s)', function () {
                ...
                tx.executeSql('CREATE TABLE test (name, id)', [], function () {
                  tx.executeSql('INSERT INTO test VALUES (?, "id1")', ['\u0000foo'], function () {
                    tx.executeSql('SELECT hex(name) AS `hex` FROM test', [], function (tx, res) {
                      ...
                      var expected = ['000066006F006F00', '00666F6F'];
                      ok(
                        expected.indexOf(hex) !== -1,
                        'hex matches: ' + JSON.stringify(hex) + ' should be in ' + JSON.stringify(expected)
                      );
                      ...
                      tx.executeSql('SELECT hex("foob") AS `hex`', [], function (tx, res) {
                        ...
                        equal(hex.length, otherHex.length, 'expect same length, i.e. same global db encoding');
                ...
                tx.executeSql('CREATE TABLE test (name, id)', [], function () {
                  tx.executeSql('INSERT INTO test VALUES (?, "id1")', ['\u0000foo'], function () {
                    tx.executeSql('SELECT name FROM test', [], function (tx, res) {
                      ...
                      if (isWebSql) {
                        ok(
                          expected.indexOf(name) === -1,
                          'field value: ' +
                            JSON.stringify(name) +
                            ' should not be in this until a bug is fixed ' +
                      ...
                      ok(
                        expected.indexOf(name) !== -1,
                        'field value: ' + JSON.stringify(name) + ' should be in ' + JSON.stringify(expected)
                      );
        ...
        test_it(suiteName + ' handles UNICODE \\u2028 line separator correctly [in database]', function () {
          if (isWP8) pending('BROKEN for WP(8)'); // [BUG #202] UNICODE characters not working with WP(8)
