...
function getFilesystemFromURL(url) {
  url = url.replace(msapplhRE, 'ms-appdata:///');
  var res;
  if (url.indexOf('file:/') === 0) res = getFilesystemFromPath(pathFromURL(url));
  else {
    var allfs = getAllFS();
    ...
    getFileFromPathAsync(path).done(
      ...
      function (err) {
        ...
        getFolderFromPathAsync(path).done(
          function (dir) {
            if (!ops.getContent) complete({ folder: dir });
            else
              WinJS.Promise.join({
          ...
          function (err) {
            if (err.number == WinError.fileNotFound || err.number == WinError.invalidArgument) complete({});
            else failed(FileError.NOT_READABLE_ERR);
          }
        );
      }
    );
  ...
  return new WinJS.Promise(function (complete, failed) {
    WinJS.Promise.join({
      ...
      var movefolders = function () {
        ...
        moveFolder(the.folders[todo], dst).done(movefolders, failed);
      };
