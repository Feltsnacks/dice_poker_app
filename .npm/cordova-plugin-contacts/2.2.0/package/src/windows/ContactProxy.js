  ...
  if (jobInfo) {
    ...
    for (var j = 0; j < jobInfo.size; j++) {
      ...
      contact.organizations.push(
        new ContactOrganization(false, null, rawJob.companyName, rawJob.department, rawJob.title)
      );
    }
  }
    ...
    if (eligiblePhotos.length > 0) {
      ...
      try {
        ...
        if (/^([a-z][a-z0-9+\-.]*):\/\//i.test(path)) {
          streamRef = Windows.Storage.Streams.RandomAccessStreamReference.createFromUri(
            new Windows.Foundation.Uri(path)
          );
        } else {
...
module.exports = {
  pickContact: function (win, fail, args) {
    ...
    pickRequest.done(function (contact) {
      ...
      contactsNS.ContactManager.requestStoreAsync().done(
        function (contactStore) {
          contactStore.getContactAsync(contact.id).done(
            function (con) {
  ...
  save: function (win, fail, args) {
    ...
    contactsNS.ContactManager.requestStoreAsync(contactsNS.ContactStoreAccessType.appContactsReadWrite)
      .then(function (store) {
        return store.findContactListsAsync().then(
          function (lists) {
            if (lists.length > 0) {
              return lists[0];
            } else {
  ...
  remove: function (win, fail, args) {
    ...
    WinJS.Promise.join([
      contactsNS.ContactManager.requestStoreAsync(contactsNS.ContactStoreAccessType.appContactsReadWrite),
      contactsNS.ContactManager.requestStoreAsync(contactsNS.ContactStoreAccessType.allContactsReadOnly),
    ])
