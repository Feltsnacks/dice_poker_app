  ...
  describe('Contacts (navigator.contacts)', function () {
    ...
    describe('save method', function () {
      ...
      it('contacts.spec.21 should be able to save a contact', function (done) {
        ...
        navigator.contacts.create(obj).save(saveSuccess, saveFail);
      });
      ...
      it(
        ...
        function (done) {
          ...
          navigator.contacts.create(contact).save(saveSuccess, saveFail);
        },
        HIGH_TIMEOUT
      );
    });
    ...
    describe('Contact.remove method', function (done) {
      ...
      it(
        ...
        function (done) {
          ...
          var contact = new Contact(
            'this string is supposed to be a unique identifier that will never show up on a device'
          );
    ...
    describe('Round trip Contact tests (creating + save + delete + find)', function () {
          ...
          saveAndFindBy(
            ...
            function (savedContact) {
              savedContact.remove(
                ...
                function (e) {
                  throw (
                    "Newly created contact's remove function invoked error callback. Test failed: " + JSON.stringify(e)
                  );
                }
              );
            },
              ...
              savedContact.remove(function () {
                ...
                var findWin = function (seas) {
                  ...
                  savedContact.remove(
                    function (e) {
                      throw (
                        'Success callback called after non-existent Contact object called remove(). Test failed: ' +
                        JSON.stringify(e)
                      );
                    },
                    function (e) {
      ...
      it(
        'contacts.spec.31 Find should return a contact with correct birthday field type',
        function (done) {
          // Save method is not supported on Windows platform
          if (isWindows || isWindowsPhone8 || isIOSPermissionBlocked) {
            pending();
      ...
      it(
        'contacts.spec.32 Find should return a contact with correct IM field',
        function (done) {
          // Save method is not supported on Windows platform
          if (isWindows || isWindowsPhone8 || isIOSPermissionBlocked) {
            pending();
...
exports.defineManualTests = function (contentEl, createActionButton) {
        ...
        if (contacts.length === 0) {
          ...
          for (var i = 0; i < contacts.length; i++) {
            ...
            var contactNameTag = contact.name
              ? '<tr><td>' + contact.name.formatted + '</td><td>'
              : '<tr><td>(No Name)</td><td>';
            s = s + contactNameTag;
  ...
  function pickContact() {
    ...
    navigator.contacts.pickContact(
      ...
      function (e) {
        results.innerHTML =
          e && e.code === ContactError.NOT_SUPPORTED_ERROR
