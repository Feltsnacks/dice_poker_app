...
describe('Contacts Android', function () {
  ...
  function pickContact(name) {
    ...
    return driver
      ...
      .executeAsync(
        function (pID, cb) {
          navigator._appiumPromises[pID].promise.then(
            function (contact) {
              cb(contact);
            },
            function (err) {
  ...
  function renameContact(oldName, newGivenName, newFamilyName) {
    return driver
      ...
      .executeAsync(
        function (oldname, newgivenname, newfamilyname, callback) {
  ...
  it(
    'contacts.ui.util configuring driver and starting a session',
    function (done) {
      getDriver().fail(fail).done(done);
    },
    5 * MINUTE
  );
  ...
  describe('Picking contacts', function () {
    afterEach(function (done) {
      removeTestContacts().finally(done);
    }, MINUTE);

    it(
      'contacts.ui.spec.1 Pick a contact',
      function (done) {
        driver
          .then(function () {
    ...
    it(
      'contacts.ui.spec.2 Update an existing contact',
      function (done) {
        driver
          .then(function () {
    ...
    it(
      'contacts.ui.spec.3 Create a contact with no name',
      function (done) {
        driver
          .then(function () {
            return addContact();
    ...
    it(
      'contacts.ui.spec.4 Create a contact with Unicode characters in name',
      function (done) {
        driver
          .then(function () {
  ...
  it(
    'contacts.ui.util Destroy the session',
    function (done) {
      driver.quit().done(done);
    },
    MINUTE
  );
});
